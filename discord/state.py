...
class GuildSubscriptions:
    ...
    def clear(self) -> None:
        ...
        self._activities: utils.SnowflakeList = utils.SnowflakeList()
    ...
    def _tick(self) -> None:
        ...
        if not self._blocked:
            self._task = self.state.loop.create_task(self._tick_task())
    @property
    def empty(self) -> bool:
    ...
    async def _requeue_subscriptions(self):
        ...
        self.clear()
        for guild_id in self.state._guilds:
            ...
            payload: gw.BaseGuildSubscribePayload = {
                ...
                'channels': channels.get(guild_id, {}),
            }
            if key in pending:
    ...
    async def _flush(self) -> None:
        ...
        if not payload:
            return
        await self.state.ws.bulk_guild_subscribe(payload)
        self._pending = {}
        for key, subscriptions in payload.items():
            ...
            for attr, value in subscriptions.items():
                if isinstance(value, bool):
                    ...
                    if value:
                        record[guild_id] = value
                    else:
                        record.pop(guild_id, None)
                else:
                    record = getattr(self, f'_{attr}')
                    if value:
                        record[guild_id] = utils.SnowflakeList(value)
                    else:
                        record.pop(guild_id, None)
...
class FakeClientPresence(Presence):
    ...
    def client_status(self) -> ClientStatus:
        ...
        return ClientStatus(status, client_status)
    @property
    def activities(self) -> Tuple[ActivityTypes, ...]:
        return getattr(self.state.current_session, 'activities', ())
...
class ConnectionState:
    ...
    def parse_ready_supplemental(self, extra_data: gw.ReadySupplementalEvent) -> None:
        ...
        for presence in extra_data['merged_presences'].get('friends', []):
            user_id = int(presence.pop('user_id'))
    ...
    def parse_passive_update_v1(self, data: gw.PassiveUpdateEvent) -> None:
        ...
        for channel_data in data.get('channels', []):
            ...
            if not channel:
                continue
            channel.last_message_id = utils._get_as_snowflake(channel_data, 'last_message_id')
            if 'last_pin_timestamp' in channel_data and hasattr(channel, 'last_pin_timestamp'):
                channel.last_pin_timestamp = utils.parse_time(channel_data['last_pin_timestamp'])
        guild._voice_states = {}
    ...
    def parse_user_achievement_update(self, data: gw.AchievementUpdatePayload) -> None:
        achievement: AchievementPayload = data.get('achievement')
    ...
    async def assert_guild_presence_count(self, guild: Guild):
        ...
        for channel in guild.channels:
            if channel.permissions_for(guild.default_role).read_messages and channel.permissions_for(guild.me).read_messages:
                break
        else:
    ...
    def parse_interaction_create(self, data: gw.InteractionEvent) -> None:
        ...
        i = Interaction._from_self(channel, type=type, user=self.user, name=name, **data)
